...
- Feed management in the browser with add, edit, delete, enable/disable, and OPML import/export. The mobile-friendly manager puts the feed inventory first, keeps Add feed and OPML tools in native disclosures, and opens Add feed automatically when the inventory is empty. Deletion uses an accessible in-row confirmation with a theme-aware destructive action instead of a native browser dialog.
