...
- Production SSRF guard blocks:
  ...
  - local/private IPv6 ranges
  - IPv4-mapped private/local IPv6
...
- Cache and rate limiter are in-memory and per-process
...
- `/api/feeds/validate` is unauthenticated and rate-limited; `/api/metrics` requires bearer auth in production
